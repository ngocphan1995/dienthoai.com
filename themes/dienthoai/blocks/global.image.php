...
if (!nv_function_exists('nv_block_global_image')) {
    ...
    function nv_block_data_config_image($module, $data_block, $lang_block) {
        ...
        foreach ($imagecontent as $key => $_imagecontent) {
            ...
            $html .= '<tr><td>Image ' . $key . ' Title: </td><td><input class="form-control w400" type="text" name="imageTitle[]"id="imageTitle' . $key . '" value="' . $_imagecontent[1] . '"/><input type="button" class="btn btn-primary" value="Browse server" name="selectimg2"/></td></tr>';
            ...
            $html .= '<tr><td>Image ' . $key . ' Description2: </td><td><input class="form-control w400" type="text" name="imageDescription2[]" id="imageDescription2' . $key . '"value="' . $_imagecontent[3] . '"/><input type="button" class="btn btn-primary" value="Browse server" name="selectimg3"/></td></tr>';
        ...
        $html .= '<script type="text/javascript">
			//<![CDATA[
				$("input[name=selectimg2]").click(function(){
					var area = $(this).prev().attr("id");
					var path= "' . NV_UPLOADS_DIR . '/slides";
					var type= "image";
					nv_open_browse(script_name + "?" + nv_name_variable + "=upload&popup=1&area=" + area+"&path="+path+"&type="+type, "NVImg", "850", "420","resizable=no,scrollbars=no,toolbar=no,location=no,status=no");
        ...
        $html .= '<script type="text/javascript">
			//<![CDATA[
				$("input[name=selectimg3]").click(function(){
					var area = $(this).prev().attr("id");
					var path= "' . NV_UPLOADS_DIR . '/slides";
					var type= "image";
					nv_open_browse(script_name + "?" + nv_name_variable + "=upload&popup=1&area=" + area+"&path="+path+"&type="+type, "NVImg", "850", "420","resizable=no,scrollbars=no,toolbar=no,location=no,status=no");
    ...
    function nv_block_data_config_image_submit($module, $lang_block) {
        ...
        $imageDescription2 = $nv_Request->get_array('imageDescription2', 'post', '', '');
        ...
        for ($i = 0; $i < 10; ++$i) {
            ...
            $images[] = $img . "|" . $imageTitle[$i] . "|" . $imageDescription[$i]. "|" . $imageDescription2[$i];
        }
    ...
    function nv_block_global_image($block_config, $block_theme) {
        ...
        for ($i = 0; $i < $imgcount; ++$i) {
            if (!empty($images[$i]['imgpath'])) {
               ...
               if (!empty($images[$i]['imgdescription2'])) {

                    $xtpl->parse('main.img.style_' . $j . '.imgdescription2');
                }
